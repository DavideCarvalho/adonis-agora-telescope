...
description: The mental model behind Telescope — what a watcher is, the uniform Entry shape every recording produces, the generic diagnostics spine that records all library events, exception auto-capture, and how trace correlation works without coupling to @agora/context.
---
...
core watchers in `@agora/telescope` and more in `@agora/telescope-watchers`:

| Watcher                | Package                      | Entry type   | Records                                              |
| ---------------------- | ---------------------------- | ------------ | --------------------------------------------------- |
| Request                | `@agora/telescope`           | `request`    | Each inbound HTTP request — method, url, status, duration. |
| Exception              | `@agora/telescope`           | `exception`  | Any error thrown by the HTTP pipeline (auto), or recorded manually. |
| Diagnostics            | `@agora/telescope`           | `diagnostic` | Every `agora:<lib>:<event>` publish.                 |
| Lucid query            | `@agora/telescope-watchers`  | `query`      | Each `db:query` — SQL, bindings, duration, connection. |
| Mail                   | `@agora/telescope-watchers`  | `mail`       | Each `mail:sent` — mailer, from, to, subject.        |
| Cache                  | `@agora/telescope-watchers`  | `cache`      | `@adonisjs/cache` hit / miss / write / delete / clear. |

The built-in request watcher runs as **server middleware**; the diagnostics watcher
is started by the core provider at boot; the per-technology watchers subscribe to the
AdonisJS event emitter. They all share one rule:
...
The built-in `EntryType` constant names the recorded types — `Request`, `Diagnostic`,
`Query`, `Exception`, `Mail`, `Cache` — plus reserved values (`Job`, `Redis`, `Event`,
`Log`) so a future watcher records under a stable, already-documented type without a
breaking change.
...
the runtime slot (no DI), is a no-op when Telescope is off, and **never throws**:
...
library, **one** `DiagnosticsWatcher` records every event any `@agora/*` library
emits through [`@agora/diagnostics`](/docs/diagnostics) — one `diagnostic` entry per
...
exactly what makes `@agora/diagnostics` start building and publishing envelopes at all
(zero overhead when nobody is listening).

```text
@agora/billing  ──publish──▶  agora:billing:invoice-paid  (node:diagnostics_channel)
                                        │
                                        ▼
...
Telescope is a separate repo and has **no `@agora/*` dependencies** — a hard dep would
...
When [`@agora/context`](/docs/context) is present, every entry recorded inside a
