...
An extension lets a sibling library — say `@agora/durable` — add a first-class
**Workflows** page to the Telescope dashboard, complete with stat cards, time series and
tables backed by its own data, **without** `@agora/telescope` knowing anything about it
...
```ts
import { defineTelescopeExtension } from '@agora/telescope'
