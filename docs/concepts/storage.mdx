...
- **Single pooled connection.** Writes are serialized (single-flight), so the store holds
  at most one connection from the pool at a time — a watcher-event burst can't exhaust the
  app's DB pool.
- **Integer durations.** `duration_ms` is rounded to an integer at the store (it's an
  INTEGER column), so watchers timing with `performance.now()` never produce a fractional
  value Postgres would reject.
...
Telescope's core already ships a background pruner: set `prune: { after: '24h', keepLast?,
intervalMs? }` in `config/telescope.ts` and it deletes stale entries on a timer (default
