...
[`@agora/telescope/ai`](/docs/telescope/packages/ai).

## Providers registered by `configure`

For reference, `node ace configure @agora/telescope` registers the core provider and — for
each optional feature you pick at the prompt — its provider in `adonisrc.ts`. Every provider
is a subpath of the one `@agora/telescope` package:

| Feature   | Provider import path                     | Extra wiring                            |
| --------- | ---------------------------------------- | --------------------------------------- |
| core      | `@agora/telescope/telescope_provider`    | `TelescopeMiddleware` on `server` stack |
| watchers  | `@agora/telescope/watchers_provider`     | —                                       |
| ui        | `@agora/telescope/ui_provider`           | routes registered at boot               |
| alerts    | `@agora/telescope/alerts_provider`       | —                                       |
| ai        | `@agora/telescope/ai_provider`           | binds `TelescopeAiDiagnoser`            |

Storage is **not a separate package** — the `memory` and `lucid` drivers are built into
`@agora/telescope` and selected in `config/telescope.ts`. `node ace configure @agora/telescope`
