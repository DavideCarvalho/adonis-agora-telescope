...
| `otel`       | `OtelConfig`                     | _disabled_                    | [OTel export](/docs/telescope/packages/otel) — ships recorded entries as OTLP spans/logs to a Collector. Off by default; its `@opentelemetry/*` peers are only imported when enabled. |
