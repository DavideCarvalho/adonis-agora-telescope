...
A sampled-out entry is never persisted, and the dashboard's
[retention indicator](/docs/telescope/packages/telescope-ui#getting-around) tells viewers which
types are recording below 100% so nobody reads a sampled count as the real one.

Reach for a custom store only when the decision needs something sampling can't express — a
per-URL rule, a lookup, a rewrite.
...
exactly what the built-in [`sampling`](#sampling-is-already-built-in) config's
