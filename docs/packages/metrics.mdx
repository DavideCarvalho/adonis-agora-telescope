...
[`TelescopeService`](/docs/telescope/packages/core#headless-api) as `telescopeStore` — plus
optional tuning:
...
This used to be split, and the split was invisible until it mattered: the alert poller read
both types while the metrics side counted only the server one, so a front-end-only incident
could page you on Slack and still render "No exceptions recorded 🎉" on the dashboard it
linked to. Fixed in 0.12.0.
</Callout>
