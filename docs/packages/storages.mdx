...
Storage is **not a separate package** — it lives in `@agora/telescope` as a config-driven
...
The `lucid` driver needs a table. `node ace configure @agora/telescope` publishes the
migration stub alongside the config file:

```sh
node ace configure @agora/telescope   # publishes config/telescope.ts + the migration
...
import { LucidTelescopeStore } from '@agora/telescope'
