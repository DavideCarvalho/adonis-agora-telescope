...
`@adonisjs/lucid`, `@adonisjs/mail`, `@adonisjs/cache`, `@adonisjs/queue` and
`@adonisjs/redis` are all **optional** peers — install only the ones whose watcher you
enable. With none installed the package still loads; the watchers simply have nothing to
subscribe to (the queue and events watchers idle on a channel/emitter nobody feeds, and the
redis watcher no-ops on a `null` manager).
