...
payload, outcome, attempts and duration, correlated to the active trace. This is an
**optional peer** watcher: with `@adonisjs/queue` absent it is a pure no-op.
...
trace. Like the queue watcher this is an **optional peer** watcher: with `@adonisjs/redis`
absent it no-ops.
...
so either major version works — a real compatibility gap in `@adonis-agora/durable`'s own
`cron-parser` usage (which only calls the v4 shape) that this watcher does not repeat.
</Callout>
...
`@adonisjs/lucid`, `@adonisjs/mail`, `@adonisjs/cache`, `@adonisjs/queue`, `@adonisjs/redis`
and `cron-parser` are all **optional** peers — install only the ones whose watcher/capability
you enable. With none installed the package still loads; the watchers simply have nothing to
subscribe to (the queue and events watchers idle on a channel/emitter nobody feeds, the redis
watcher no-ops on a `null` manager, the Live Queue Manager reports `configured: false`, and
Live Schedules' `nextRunAt` is always `null`).
