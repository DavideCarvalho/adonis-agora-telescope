...
description: The watchers subpath of @agora/telescope — record every Lucid SQL query (sql, bindings, duration, connection), every email sent, @adonisjs/cache hit/miss/write/delete events, outbound fetch calls, and AdonisJS logger output, each correlated to the active request trace.
---

import { Tab, Tabs } from 'fumadocs-ui/components/tabs'
import { Callout } from 'fumadocs-ui/components/callout'

The core records requests, exceptions and diagnostics. The **`@agora/telescope/watchers`**
...
Everything lives in the one `@agora/telescope` package — there is nothing extra to install.
...
node ace configure @agora/telescope   # then pick "Watchers" at the prompt
```
...
Selecting **Watchers** registers `@agora/telescope/watchers_provider` in `adonisrc.ts` and
...
| `watchers` | `['query']` | Which watchers run — any of `'query'`, `'mail'`, `'cache'`, `'http-client'`, `'logs'`. |
...
restores the original `fetch` on shutdown.
