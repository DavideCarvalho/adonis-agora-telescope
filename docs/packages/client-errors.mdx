...
<Callout type="warn">
The dashboard's **Exceptions** section and the type filter are separate things: filtering the
entries list by `type: exception` shows server exceptions only, because that is literally the
type you asked for. Browser errors live under `client_exception`. The grouped Exceptions view
and the overview span both.
</Callout>
