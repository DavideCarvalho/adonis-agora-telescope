// Ported near-verbatim from `nestjs-telescope`'s `packages/core/src/profiling/types.ts` — the V8
// inspector Profiler protocol shapes and the aggregated flame tree are framework-agnostic (pure
// `node:inspector` + arithmetic, no NestJS/AdonisJS dependency at all).
 ...
 * lazily and ONLY when profiling is enabled — see {@link CpuProfiler} in
 * `cpu-profiler.ts`).
 *
