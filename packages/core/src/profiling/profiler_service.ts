...
import { safeRecord } from '../watchers/record.js';
import { CpuProfiler, type CpuProfilerResult } from './cpu-profiler.js';
import type { ResolvedTelescopeCpuProfilingConfig } from './define_config.js';
...
export interface ProfilerServiceDeps {
  ...
  /** Records the finished profile; injectable for tests. Defaults to `safeRecord`
   *  through the runtime store slot (mirrors every other watcher). */
 ...
 * it. {@link TelescopeMiddleware} calls {@link shouldProfile} (cheap gate), then
 * {@link begin}/{@link end} around the request body — ported near-verbatim from
 * `nestjs-telescope`'s `ProfilerService`; only the recording seam changed (fire-and-forget
 * `safeRecord` through the runtime store slot, matching every other Adonis watcher, instead of a
 * NestJS-injected recorder).
 *
...
export class ProfilerService {
  ...
  private readonly recordFn: (input: RecordInput<CpuProfileContent>) => void;

  constructor(
    private readonly config: ResolvedTelescopeCpuProfilingConfig,
    deps: ProfilerServiceDeps = {},
  ) {
    ...
    this.recordFn = deps.record ?? ((input) => safeRecord(input, 'ProfilerService'));
  }
   ...
   */
  arm(arm: { count: number; label?: string }): { pendingManual: number } {
