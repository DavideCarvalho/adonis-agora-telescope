 ...
 * client-error endpoint. Per-process by design (no cross-process coordination):
 * in a multi-instance deployment the same client may get up to `perMinute`
 * requests PER INSTANCE. The intent is cheap abuse-dampening on a public
 * surface, not a hard global quota (a shared limiter would need a cross-process
 * store, out of scope for this in-process slice).
 *
 ...
 *
 * Ported from the NestJS `nestjs-telescope` core client-error rate limiter.
 */
