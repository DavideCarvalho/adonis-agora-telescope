...
/**
 * Framework-light `dashboardAuth` mechanism for the telescope dashboard — config resolution, the
 * open-redirect guard, the login decision, session verification, and the composed session-guard
 * decision. NO AdonisJS types leak in (mirroring `guard.ts`/`http.ts`), so every branch is
 * unit-testable against a plain object with no HTTP server. The Adonis glue in `dashboard_auth.ts`
 * adapts an `HttpContext` to these functions and owns the cookie read/write via AdonisJS's native
 * cookie API.
 *
 * When configured it gates BOTH the dashboard SPA shell (a full-page navigation redirected `302`
...
/**
 * Author-facing `dashboardAuth` option on `config/telescope_ui.ts`. Gates the dashboard behind a
 * signed session cookie via the built-in server-rendered login page (`GET <path>/login`) — the
 * bundled dashboard SPA stays untouched.
 *
 * `login` is just a function that may be `async` and close over `app.container`/services (import
 * them at the top of `config/telescope_ui.ts`), so there is nothing extra to build for the async
 * case.
 */
...
export const SESSION_COOKIE_NAME = 'telescope_dashboard_session';
...
export function resolveDashboardAuth(
  ...
  if (typeof options.secret !== 'string' || options.secret === '') {
    throw new Error(
      'telescope_ui: dashboardAuth.secret is required and must be a non-empty string ' +
        '(HMAC-SHA256 signing key, 32+ bytes recommended). Failing closed.',
    );
  }
  if (typeof options.login !== 'function') {
    throw new Error('telescope_ui: dashboardAuth.login is required (a login hook).');
  }
  return {
 ...
 * root-relative path (`/telescope/foo`), never a protocol-relative (`//evil.com`) or absolute
...
/**
 * The framework-light outcome of the composed session guard, mapped to HTTP by the Adonis glue:
 * - `allow`        → let the request through (valid session, or `dashboardAuth` unconfigured).
 * - `redirect`     → a `page` navigation with no valid session: `302` to the login page carrying
 *   `returnTo` (the original, server-supplied request URL).
 * - `unauthorized` → an `api` request with no valid session: a plain `401`.
 */
export type DashboardAuthDecision =
  | { kind: 'allow' }
  | { kind: 'redirect'; returnTo: string }
  | { kind: 'unauthorized' };

/**
 * Decide the session guard for a dashboard resource with NO HTTP types. Runs AFTER the existing
 * `authorize` hook (they compose — both must pass). When `auth` is `null` (`dashboardAuth`
 * unconfigured) it always allows, so behavior is byte-for-byte unchanged. Otherwise a valid session
 * allows; a `page` request without one redirects (carrying `requestUrl` as `returnTo`) and an `api`
 * request without one is unauthorized.
 */
export function decideDashboardAuth(
  ...
  if (auth === null) return { kind: 'allow' };
  const session = readSession(auth, cookieValue, now);
  if (session !== null) return { kind: 'allow' };
  if (mode === 'page') return { kind: 'redirect', returnTo: requestUrl };
