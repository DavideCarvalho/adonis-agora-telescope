...
describe('recordRequest', () => {
  it('records a request entry with method, url, status, duration', () => {
    const store = new InMemoryTelescopeStore();
    const startedAt = Date.now() - 25;
    recordRequest(store, stubCtx('get', '/users/42', 200), startedAt);

    const entries = store.list({ type: EntryType.Request });
  ...
  it('strips the query string from the url', () => {
    const store = new InMemoryTelescopeStore();
    recordRequest(store, stubCtx('GET', '/search?q=hello&p=2', 200), Date.now());
    const content = store.list()[0]?.content as { url: string };
    expect(content.url).toBe('/search');
  });

  it('handles a missing status code as null (no status tag)', () => {
    const store = new InMemoryTelescopeStore();
    recordRequest(store, stubCtx('POST', '/webhook'), Date.now());
  ...
  it('uses an explicit duration override', () => {
    const store = new InMemoryTelescopeStore();
    recordRequest(store, stubCtx('GET', '/', 204), Date.now(), { durationMs: 999 });
    const entry = store.list()[0];
    expect(entry?.durationMs).toBe(999);
    expect((entry?.content as { durationMs: number }).durationMs).toBe(999);
  });

  it('records an explicit traceId onto the entry and content', () => {
    const store = new InMemoryTelescopeStore();
    recordRequest(store, stubCtx('GET', '/', 200), Date.now(), { traceId: 'tr-9' });
    const entry = store.list()[0];
