// Ported near-verbatim from `nestjs-telescope`'s `packages/ui/src/react/components/flamegraph.ts` —
// pure layout math over a {@link FlameNode} tree, framework-agnostic (no NestJS/AdonisJS specifics,
// only the `FlameNode` import path changed to this package's client types).
 ...
 * so a large profile doesn't emit thousands of sub-pixel divs.
 */
